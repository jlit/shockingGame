...
const int LEDPin = 19;
const int shockRelayPin = A5;
const int modePins[4] = {8, 9, 10, 11}; // Reaction, Recall, Roulette, Random
const int buttonPins[MAX_PLAYERS] = {2, 3, 4, 5, 6};
const int shockPins[MAX_PLAYERS] = {A4, A3, A2, A1, A0};
const int buzzer = 13; 
  ...
  for(int pin = 0; pin < 4; pin++) {
    if(digitalRead(modePins[pin]) == LOW) {
      if(pin != static_cast<int>(currentMode)) {
        ...
        Serial.println(pin);
        changed = true;
        currentMode = static_cast<Mode>(pin); // 0 = Reaction, 1 = Recall, 2 = Roulette, 3 = Random
      }
    }
  }
  if(changed) {
...
void shockCheater(int playerNum) {
  ...
  digitalWrite (shockRelayPin, HIGH);
  digitalWrite (shockPin, HIGH);
  delay(1000); // cheaters get a full second
  digitalWrite (shockPin, LOW);
  digitalWrite (shockRelayPin, LOW);
...
void LEDsRandomPlayer(CRGB color) {  
  int player = random(numberOfPlayers);
  if(color == CRGB::Black) { // If we passed in black then use the Recall game color for each player
...
void setup() {
  ...
  for(int modePin = 0 ; modePin < 4; modePin++) {
    pinMode(modePins[modePin], INPUT_PULLUP);
  }
  pinMode(buttonNumberOfPlayersPin, INPUT_PULLUP);
  for(int buttonPin = 0 ; buttonPin < MAX_PLAYERS; buttonPin++) {
    pinMode(buttonPins[buttonPin], INPUT_PULLUP);
  }
  pinMode(shockRelayPin, OUTPUT);
  for(int shockPin = 0 ; shockPin < MAX_PLAYERS; shockPin++) {
...
void loop() {
  ...
  while (digitalRead(buttonStartPin) == HIGH) {
    ...
    if(currentMode == RECALL) {
      LEDsRandomPlayer(CRGB::Black);  // Actually uses playeColors
    }
    if(currentMode == ROULETTE) {      
      LEDsAllRadial(CRGB::Blue);
    }
    if(currentMode == RANDOM) {
      LEDsRandomPlayer(CRGB::Cyan);
    }
    if(settingsChanged()) {
      reset();
    }
  ...
  if(currentMode == RANDOM) {
    Serial.println (F("Starting Random mode..."));
    ...
    if(currentState == PLAYING) {
      shockPlayer(random(numberOfPlayers));
    }
    reset();
  }
